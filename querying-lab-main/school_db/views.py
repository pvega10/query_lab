...
def problem_two(request):
  ...
  teachers = Instructor.objects.filter(hire_date__lt = "2010-01-01").order_by("hire_date")
...
def problem_three(request):
  ...
  teacher_name = Instructor.objects.get(id = "2")
  courses = Course.objects.filter(instructor_id = "2")

  
  print (f"Instructor: {teacher_name.first_name} {teacher_name.last_name}")
  print ("Courses:")
...
def problem_five(request):
  ...
  new_student = Student.objects.create(first_name= "Paul", last_name= "Vega", year= 2022, gpa= 3.0)
...
def problem_six(request):
    ...
    student_update = Student.objects.filter(pk=student_id).update(gpa=3.5)
